...
| 権限 | 用途 | 推奨設定 |
|------|------|----------|
| **位置情報** | 現在地・速度・進行方向の取得 | **常に許可** |
| **写真ライブラリ** | 道の駅・カントリーサインのフォトアルバム | 使用時のみ許可 |

> **補足:** 位置情報を「常に許可」に設定すると、運転中にバックグラウンドでも位置情報を取得し続け、スリープ明けや他アプリ使用後でもスムーズに再開できます。
...
![地図画面](./images/ios_001.png)

### 画面の構成要素

| 位置 | 要素 | 説明 |
|------|------|------|
| 左上 | **設定ボタン（⚙）** | [設定画面](#設定)を開きます |
| 左下 | **速度表示パネル** | 現在の速度（km/h）と天気情報を表示 |
| 右下（縦並び） | **4つのコントロールボタン** | 下記参照 |
...
|----------|------|
| ☰ リスト | [リスト画面](#リスト画面)を開く |
| 📍 道の駅マーカー | 道の駅マーカーの表示/非表示を切り替え |
| 🚩 カントリーサインマーカー | カントリーサインマーカーの表示/非表示を切り替え |
| 📍 現在地 | 地図を現在地に戻す |
...
> **補足:** 手動で地図を拡大・縮小したり、現在地ボタンをタップすると、**30秒間オートズームが一時停止**します。停止中は手動操作が優先されます。
...
アプリは自動的に **現在地から120km以内** の道の駅を検索し、地図上にマーカーで表示します。
...
地図右下の ☰ ボタンから [リスト画面](#リスト画面) を開き、地域や都道府県、市町村を絞り込んで道の駅を探せます。
...
詳細画面の下部にあるナビボタンから、以下のアプリで道の駅までの案内を開始できます。

![外部ナビアプリ選択](./images/ios_003.png)

| アプリ | 備考 |
|--------|------|
| Apple マップ | 標準搭載、追加インストール不要 |
| Google Maps | 別途インストールが必要 |
| Yahoo!カーナビ | 別途インストールが必要 |
| Waze | 別途インストールが必要 |

> **補足:** インストールされていないアプリは自動的にメニューから非表示になります。

### 現在地を共有

画面下部の共有ボタンから、iOS標準の共有シートで道の駅の位置情報を他のアプリに送信できます。

### フォトアルバム機能

各道の駅ごとに最大3枚まで写真を保存できます。

![フォトアルバム](./images/ios_004.png)

1. **写真の追加**: アルバムの空きスロット（+ アイコン）をタップ → 写真ライブラリから選択
2. **写真の表示**: サムネイルをタップ → フルスクリーン表示（左右スワイプで切り替え）
3. **写真の削除**: サムネイルを長押し → 削除を確認

> **重要:** 写真は端末内に保存されます（iCloud 同期はされません）。アプリをアンインストールすると写真は失われます。
...
地図画面右下の 🚩 ボタンで表示/非表示を切り替えます。有効にすると、地図上に市町村のカントリーサインマーカーと市町村境界が表示されます。

### カントリーサインマーカーをタップする

マーカーをタップすると、カントリーサインの詳細画面がボトムシートで開きます。

![カントリーサイン詳細画面](./images/ios_005.png)
...
![ランダムカードドロー画面](./images/ios_006.png)
...
地図画面右下の ☰ ボタンからリスト画面を開きます。

### タブ切り替え

上部のセグメントピッカーで **道の駅** と **カントリーサイン** を切り替えます。カントリーサインタブは、カントリーサインマーカーが表示されている時のみ利用可能です。

### 道の駅タブ

| サブタブ | 内容 |
|----------|------|
| **すべての道の駅** | 地域 → 都道府県 → 市町村 → 道の駅 の4階層で絞り込み |
| **お気に入り** | ハートを付けた道の駅 |
| **訪問済み** | 訪問済みの道の駅 |

![リスト画面（道の駅タブ）](./images/ios_007.png)
...
![リスト画面（カントリーサインタブ）](./images/ios_008.png)

---

## 設定

地図画面左上の ⚙ ボタンから設定画面を開きます。

![設定画面](./images/ios_009.png)

### 地図タイルの選択

| タイル | 説明 |
|--------|------|
| **Apple マップ** | iOS 標準の地図（デフォルト、最速） |
| **GSI 淡色** | 国土地理院の淡色地図（見やすい色合い） |
| **GSI 標準** | 国土地理院の標準地図 |
| **GSI 衛星写真** | 国土地理院の航空写真 |
...
GPS 由来の瞬間速度を表示しているため、トンネル内や電波の悪い場所では誤差が出ることがあります。また、連続した位置情報の時間差から算出しているため、急加速時などに一時的にずれることがあります。

### Q. 写真はクラウドに保存されますか？

**保存されません。** 写真は端末内の Michi-navi アプリ領域に保存されます。アプリをアンインストールしたり、端末を初期化すると写真は失われます。

### Q. 道の駅マーカーが表示されません

以下を確認してください：

1. 地図右下の 📍 道の駅マーカーボタンが有効になっているか
2. 位置情報の権限が「常に許可」になっているか
3. 周辺120km以内に道の駅があるか（道の駅がない地域では表示されません）
